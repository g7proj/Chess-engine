...
fn run_benchmark_case(name: &str, board: &mut Board, depth: usize, expected: u64) {
...
fn benchmark_perft_known_positions() {
    let mut startpos: Board = Board::new();
    run_benchmark_case("startpos depth 4", &mut startpos, 4, 197_281);

    let castling_fen: &str = "r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1";
    let mut castling: Board = Board::from_fen(castling_fen).expect("valid castling FEN");
    run_benchmark_case("castling depth 1", &mut castling, 1, 26);
}
